...
trait Environments { this: HasGlobal with NonReducibles =>
    ...
    protected[Environment] class Member(val symbol: Symbol, private var _value: Tree, initialized: Boolean) {
      ...
      def panic: Unit = {
        ...
        if (symbol.isMutable) {
          _initialized = true
          _mutated = true
          _value = NonReducible(ReductionError.NoSource, _value)
        }
      }
      ...
      def merge(that: Member): Unit = {
        ...
        (changed, _value, that._value) match {
          case (true, _, _)                                    => _value = NonReducible(ReductionError.NonDeterministic, _value)
