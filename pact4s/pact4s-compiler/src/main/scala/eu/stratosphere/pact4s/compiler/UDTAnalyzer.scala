...
trait UDTAnalyzer extends TreeDSL {
  ...
  protected abstract sealed class UDTDescriptor(val tpe: Type)
  protected case class PrimitiveDescriptor(myTpe: Type, default: Literal, wrapperClass: Symbol) extends UDTDescriptor(myTpe)
  protected case class ListDescriptor(myTpe: Type, listType: Type, elem: UDTDescriptor) extends UDTDescriptor(myTpe)
  protected case class CaseClassDescriptor(myTpe: Type, ctor: Symbol, ctorTpe: Type, getters: Seq[Accessor]) extends UDTDescriptor(myTpe)
  protected case class Accessor(sym: Symbol, tpe: Type, descr: UDTDescriptor)
  ...
  private val udts = collection.mutable.Map[Type, UDTDescriptor]()

  protected def analyzeType(tpe: Type): Either[String, UDTDescriptor] = {
