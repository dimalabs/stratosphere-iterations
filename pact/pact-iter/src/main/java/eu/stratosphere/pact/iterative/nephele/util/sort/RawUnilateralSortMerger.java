...
*/
public class RawUnilateralSortMerger
{	
	// ------------------------------------------------------------------------
	//                              Constants
	// ------------------------------------------------------------------------
	 ...
	 * Logging.
	 */
	private static final Log LOG = LogFactory.getLog(RawUnilateralSortMerger.class);
	 ...
	 */
	protected final List<RawNormalizedKeySorter<?>> sortBuffers;
	 ...
	 */
	protected volatile MutableObjectIterator<Value> iterator;
	 ...
	 *                                   perform the sort.
	 */
	public RawUnilateralSortMerger(MemoryManager memoryManager, IOManager ioManager,
			long totalMemory, int maxNumFileHandles, TypeAccessorsV2<Value> accessor,
			MutableObjectIterator<Value> input, AbstractInvokable parentTask, float startSpillingFraction)
	throws IOException, MemoryAllocationException
	{
		this(memoryManager, ioManager, totalMemory, -1, -1, maxNumFileHandles, accessor,
	...
	public RawUnilateralSortMerger(MemoryManager memoryManager, IOManager ioManager,
			...
			TypeAccessorsV2<Value> accessor, MutableObjectIterator<Value> input, AbstractInvokable parentTask, float startSpillingFraction)
	throws IOException, MemoryAllocationException
	{
		// sanity checks
		if (memoryManager == null | ioManager == null | accessor == null) {
		...
		this.sortBuffers = new ArrayList<RawNormalizedKeySorter<?>>(numSortBuffers);
		...
		for (int i = 0; i < numSortBuffers; i++)
		{
			...
			final RawNormalizedKeySorter<Value> buffer = new RawNormalizedKeySorter<Value>(accessor.duplicate(), sortSegments);
	...
	public void close()
	{
		...
		try {
			...
			try {
				...
				for (RawNormalizedKeySorter<?> sorter : this.sortBuffers) {
	...
	protected ThreadBase getReadingThread(ExceptionHandler<IOException> exceptionHandler,
			MutableObjectIterator<Value> reader, CircularQueues queues, AbstractInvokable parentTask,
			long startSpillingBytes)
	{
		return new ReadingThread(exceptionHandler, reader, queues, parentTask, startSpillingBytes, accessor);
	}
	 ...
	 */
	public MutableObjectIterator<Value> getIterator() throws InterruptedException
	{
		synchronized (this.iteratorLock) {
	 ...
	 */
	protected final void setResultIterator(MutableObjectIterator<Value> iterator)
	{
		synchronized (this.iteratorLock) {
	 ...
	 */
	protected final RawMergeIterator getMergingIterator(final List<Channel.ID> channelIDs,
	...
	throws IOException
	{
		...
		final List<MutableObjectIterator<Value>> iterators = new ArrayList<MutableObjectIterator<Value>>(channelIDs.size());
		...
		return new RawMergeIterator(iterators, accessor.duplicate());
	}
	...
	throws IOException
	{
		...
		final RawMergeIterator mergeIterator = getMergingIterator(channelIDs, readBuffers, channelAccesses);
		...
		Value rec = accessor.createInstance();
	...
	{
		final int id;
		final RawNormalizedKeySorter<Value> buffer;
		...
		public CircularElement(int id, RawNormalizedKeySorter<Value> buffer2) {
			this.id = id;
			this.buffer = buffer2;
		}
	}
	...
	protected static class ReadingThread extends ThreadBase
	{
		 ...
		 */
		private final MutableObjectIterator<Value> reader;
		...
		public ReadingThread(ExceptionHandler<IOException> exceptionHandler,
				MutableObjectIterator<Value> reader, CircularQueues queues,
		...
		{	
			final MutableObjectIterator<Value> reader = this.reader;
			
			final Value current = accessor.createInstance();
			Value leftoverRecord = null;
			...
			while (!done && isRunning())
			{
				...
				final RawNormalizedKeySorter<Value> buffer = element.buffer;
				if (!buffer.isEmpty()) {
	...
	protected class SpillingThread extends ThreadBase
	{
		...
		public void go() throws IOException
		{
			...
			if (cacheOnly) {
				...
				List<MutableObjectIterator<Value>> iterators = new ArrayList<MutableObjectIterator<Value>>(cache.size());
				...
				setResultIterator(iterators.size() == 1 ? iterators.get(0) :
					new RawMergeIterator(iterators, accessor.duplicate()));
				return;
			}			
			...
			final int writeBufferSize = RawUnilateralSortMerger.this.ioBufferSize;
			...
			try {
				writeBuffers = this.memoryManager.allocateStrict(RawUnilateralSortMerger.this.parent, 
			...
			try {
				...
				while (channelIDs.size() > RawUnilateralSortMerger.this.maxNumFileHandles) {
				...
				if (channelIDs.isEmpty()) {
					setResultIterator(EmptyMutableObjectIterator.<Value>get());
				}
				else {
					if (LOG.isDebugEnabled())
		...
		protected final void releaseSortBuffers()
		{
			while (!this.queues.empty.isEmpty()) {
				try {
					final RawNormalizedKeySorter<?> sorter = this.queues.empty.take().buffer;
					final List<MemorySegment> segments = sorter.dispose();
					this.memoryManager.release(segments);
					RawUnilateralSortMerger.this.sortBuffers.remove(sorter);
				}
				catch (InterruptedException iex) {
					if (isRunning()) {
	 ...
	 */
	protected static final class ChannelReaderIterator implements MutableObjectIterator<Value>
	{
	...
	public static final class InputDataCollector
	{
		private final CircularQueues queues;		// the queues used to pass buffers
		
		private RawNormalizedKeySorter<Value> currentBuffer;
